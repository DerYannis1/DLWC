...
class LitWeatherCNN(LightningModule):
    def __init__(
        self,
        variables,
        lr,
        ...
        self.model = SimpleWeatherCNN(
            in_channels=len(variables),
            out_channels=len(variables),
        )
        self.loss_fn = F.mse_loss

    def forward(self, x_cerra, time_interval):
        # CNN nutzt nur x_cerra
        return self.model(x_cerra)

    def _shared_step(self, batch, stage):
        x_cerra, y = batch
        # time_interval wird ignoriert für das CNN
        y_hat = self.model(x_cerra)
