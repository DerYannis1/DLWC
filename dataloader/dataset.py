# dataset.py (modifications to TrainDataset only)
import os
...
class TrainDataset(Dataset):
    def __init__(
        ...
        for f in self.files:
            ...
            self.time_index.extend([(f, i) for i in range(num_times - 1)])
            ds.close()
...
class TestDataset(Dataset):
    def __init__(self, root_dir, variables: List[str], transform):
        super().__init__()
        self.root_dir = root_dir
        self.variables = variables
    ...
    def __getitem__(self, index):
        ...
        return self.transform(inp_tensor), self.transform(out_tensor), self.variables
